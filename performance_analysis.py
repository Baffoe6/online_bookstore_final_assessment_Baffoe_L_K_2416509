...
class PerformanceProfiler:
    ...
    def timeit_analysis(self, func: Callable, *args, number: int = 1000, **kwargs) -> Dict[str, float]:
...
"""
    ...
    def compare_performance(self, original_func: Callable, optimized_func: Callable, 
        ...
        time_improvement = ((original_timeit['avg_time'] - optimized_timeit['avg_time']) 
                          / original_timeit['avg_time'] * 100)
        
        return {
            'original': {
                'profile': original_profile,
                'timeit': original_timeit
            },
            'optimized': {
                'profile': optimized_profile,
                'timeit': optimized_timeit
            },
            'improvement_percentage': time_improvement,
            'speedup_factor': original_timeit['avg_time'] / optimized_timeit['avg_time']
        }
...
class PerformanceAnalyzer:
    ...
    def analyze_book_service(self):
        ...
        # Test search_books
        result = self.profiler.timeit_analysis(BookService.search_books, "Gatsby", number=1000)
        print(f"🔍 search_books: {result['avg_time']:.6f}s average")
        
        # Test get_book_by_title
        result = self.profiler.timeit_analysis(BookService.get_book_by_title, "The Great Gatsby", number=1000)
    ...
    def analyze_cart_service(self):
        ...
        cart = Cart()
        book = Book("Test Book", "Test Author", "Test Category", 10.99, "/test.jpg", "Test Description")
        
        # Test add_to_cart
        result = self.profiler.timeit_analysis(CartService.add_to_cart, cart, "Test Book", 1, number=1000)
        print(f"➕ add_to_cart: {result['avg_time']:.6f}s average")
        
        # Test update_cart_item
        result = self.profiler.timeit_analysis(CartService.update_cart_item, cart, "Test Book", 2, number=1000)
    ...
    def analyze_validation_performance(self):
        ...
        result = self.profiler.timeit_analysis(ValidationUtils.validate_email, "test@example.com", number=10000)
        print(f"📧 validate_email: {result['avg_time']:.6f}s average")
        
        # Test quantity validation
        result = self.profiler.timeit_analysis(ValidationUtils.validate_quantity, 5, number=10000)
    ...
    def analyze_user_service(self):
        ...
        def mock_register():
            return UserService.register_user("test@example.com", "password123", "Test User")
...
def demonstrate_optimization():
    ...
    # Create test data
    books = [
        Book("The Great Gatsby", "F. Scott Fitzgerald", "Fiction", 10.99, "/gatsby.jpg", "Classic novel"),
        Book("1984", "George Orwell", "Dystopia", 8.99, "/1984.jpg", "Dystopian fiction"),
        Book("I Ching", "King Wen", "Traditional", 18.99, "/iching.jpg", "Ancient text"),
        Book("Moby Dick", "Herman Melville", "Adventure", 12.49, "/moby.jpg", "Epic tale")
    ]
    
    # Compare performance
    profiler = PerformanceProfiler()
    comparison = profiler.compare_performance(inefficient_search, optimized_search, books, "Gatsby")
