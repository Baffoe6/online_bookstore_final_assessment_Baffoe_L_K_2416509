...
class CartService:
    ...
    def update_cart_item(cart: Cart, book_title: str, quantity: int) -> ServiceResult:
        ...
        try:
            validated_quantity = ValidationUtils.validate_quantity(quantity, allow_zero=True)
...
class UserService:
    ...
    @staticmethod
    def register_user(email: str, password: str, name: str, address: str = "") -> ServiceResult:
...
class OrderService:
    ...
    @staticmethod
    def calculate_discount(total_amount: float, discount_code: str) -> Tuple[float, float, str]:
        ...
        message = f"{discount_info['description']} applied! You saved ${discount_amount:.2f}"
