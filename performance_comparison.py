...
def run_performance_comparison():
    ...
    times = timeit.repeat(lambda: BookService.search_books("Gatsby"), number=1000, repeat=3)
    avg_time = sum(times) / len(times)
    ...
    times = timeit.repeat(lambda: ValidationUtils.validate_email("test@example.com"), number=10000, repeat=3)
    avg_time = sum(times) / len(times)
    print(f"  validate_email: {avg_time:.6f}s average")
    
    # Test User performance
    print("\n👤 User Performance:")
    times = timeit.repeat(lambda: User("test@example.com", "password123", "Test User"), number=10, repeat=3)
    avg_time = sum(times) / len(times)
